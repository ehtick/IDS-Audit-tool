...
public class SchemaLocationFallbackTests : BuildingSmartRepoFiles
{
    ...
    private const string ValidBody =
        "<info><title>Minimal valid IDS</title></info>" +
        "<specifications>" +
        ...
        "<requirements><entity><name><simpleValue>IFCWALL</simpleValue></name></entity></requirements>" +
        "</specification>" +
        "</specifications>";

    private static Stream ToStream(string schemaLocationAttribute)
    {
        var xml =
    ...
    {
        using var stream = ToStream(schemaLocationAttribute: string.Empty);
    ...
    {
        using var stream = ToStream(schemaLocationAttribute: " xsi:schemaLocation=\"\"");
        var options = new SingleAuditOptions()
        {
            IdsVersion = IdsVersion.Ids1_0,
            OmitIdsContentAudit = true,
    ...
    public void DoesNotOverrideAnExistingCorrectSchemaLocation()
    {
        using var stream = ToStream(
            schemaLocationAttribute: " xsi:schemaLocation=\"http://standards.buildingsmart.org/IDS http://standards.buildingsmart.org/IDS/1.0/ids.xsd\"");
        var options = new SingleAuditOptions()
        {
            IdsVersion = IdsVersion.Ids1_0,
            OmitIdsContentAudit = true,
    ...
    {
        using var stream = ToStream(
            schemaLocationAttribute: " xsi:schemaLocation=\"http://standards.buildingsmart.org/IDS http://example.com/wrong.xsd\"");
        var options = new SingleAuditOptions()
        {
            IdsVersion = IdsVersion.Ids1_0,
            OmitIdsContentAudit = true,
